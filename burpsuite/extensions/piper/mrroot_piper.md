**Piper Extension Confing** - Yet to add more custom commands
```yaml
messageViewers:
...
- prefix: [gron]
  ...
  name: gron
  filter: {prefix: '{'}
- prefix: [python3, SCRIPT_ROOT\extract-api-endpoints.py]
  inputMethod: stdin
  requiredInPath: [python3]
  exitCode: [0]
  ...
  filter:
    header:
      ...
      regex:
        ...
        flags: [case insensitive]
  scope: response_only
- prefix: [python3, SCRIPT_ROOT\extract-uuid-infos.py]
  inputMethod: stdin
  requiredInPath: [python]
  exitCode: [0]
...
- prefix: [sh, -c, 'grep -Eo ''"operationName":"[^"]+'' | tr -d '':"'' | sed ''s/operationName//''']
  inputMethod: stdin
  name: Mutuation Commentator
  filter: {prefix: '{"operationName":”'}
  scope: request_only
  applyWithListener: true
highlighters:
